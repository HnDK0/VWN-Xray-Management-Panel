...
- ✅ **VLESS + TLS + Vision** — direct connections with `xtls-rprx-vision` flow, own TLS cert on port 443, fallback to nginx stub
...
Client (router/Clash — Vision)
    └── domain:443/TCP → Xray-Vision (direct TLS) → outbound
                              ↓ fallback (non-Vision traffic)
                         127.0.0.1:7443 → Nginx → VLESS+WS stub
...
|------|---------|
| 22 | SSH (configurable) |
| 443 | VLESS+WS+TLS via Nginx (+ VLESS+Vision direct when installed) |
| REALITY_PORT | VLESS+Reality (default: 8443, external, before Stream SNI) |
...
Direct connections with `xtls-rprx-vision` flow. Separate `xray-vision` service listening on port **443**. Requires WS+TLS to be active.
...
/etc/nginx/cert/
├── cert.pem / cert.key      # WS/Vision TLS certificate (shared)
...
- ✅ **VLESS + TLS + Vision** — прямые подключения с `xtls-rprx-vision`, собственный TLS-сертификат на порту 443
...
Клиент (роутер/Clash — Vision)
    └── domain:443/TCP → Xray-Vision (прямое TLS) → outbound
                         ...
                         127.0.0.1:7443 → Nginx → сайт-заглушка

Stream SNI map (порт 443):
    ws.example.com   → 127.0.0.1:7443   (nginx HTTPS → Xray WS)
    default          → 127.0.0.1:10443  (Xray Reality)
...
Прямые подключения с потоком `xtls-rprx-vision`. Отдельный сервис `xray-vision` слушающий порт **443**. Требует WS+TLS.
...
Клиент → domain:443 → Xray-Vision (прямая TLS терминация)
                              ↓ fallback (не-Vision трафик)
                        127.0.0.1:7443 → Nginx → WS заглушка
```
...
- Не-Vision трафик пересылается на Nginx `127.0.0.1:7443` (общий с WS)
...
/etc/nginx/cert/
├── cert.pem / cert.key      # WS/Vision TLS сертификат (общий)
