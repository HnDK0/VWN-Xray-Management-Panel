...
- ✅ **Web Panel** — browser-based management with random URL path protection

## Security

- **Random panel URL** — `/<token>/<token>` (64 hex chars) — impossible to guess or brute-force
- **JWT authentication** — HMAC-SHA256 signed tokens with 8-hour TTL
- **bcrypt password hashing** — 12 rounds, secure storage
- **Rate limiting** — 5 login attempts per 15 minutes
- **CSRF protection** — tokens for write/admin commands
- **Content Security Policy** — nonce-based script execution
- **Systemd isolation** — NoNewPrivileges, IPAddressDeny, ProtectSystem=strict
- **Audit logging** — all panel actions logged to `/var/log/vwn-panel-audit.log`
- **Safe updates** — downloaded via tmpfile with shebang verification (no `curl | bash`)

## Web Panel

### Installation

The panel is installed separately via menu item 32. During installation:
1. **Random URL path** is generated automatically (e.g. `/a1b2c3.../e5f6g7...`)
2. **Password** — choose manual entry or auto-generation (16 chars)
3. **URL** is displayed after successful installation

### Access

```
https://your-domain.com/<random_path>/
```

The path is stored in `/usr/local/etc/xray/panel.conf` and can be viewed via menu item 32 → 1.

### Features

- Dashboard with service status, RAM/disk usage, uptime
- User management (add/remove/rename)
- Domain management (warp/relay/tor)
- Configuration editor (xray, nginx, reality)
...
├── diag.sh       # Diagnostics
├── menu.sh       # Main menu
├── panel.sh      # Web panel installation & management
├── web_panel.py  # Web panel backend
└── panel.html    # Web panel frontend
...
├── panel.conf               # Panel settings (secret, hash, path, port)
...
# Panel — check logs
journalctl -u vwn-panel -n 50 --no-pager
```
...
- ✅ **Веб-панель** — управление через браузер с защитой случайным URL

## Безопасность

- **Случайный URL панели** — `/<токен>/<токен>` (64 hex символа) — невозможно подобрать или перебрать
- **JWT аутентификация** — HMAC-SHA256 токены с TTL 8 часов
- **bcrypt хэширование паролей** — 12 раундов, безопасное хранение
- **Rate limiting** — 5 попыток логина за 15 минут
- **CSRF защита** — токены для write/admin команд
- **Content Security Policy** — nonce-based выполнение скриптов
- **Systemd изоляция** — NoNewPrivileges, IPAddressDeny, ProtectSystem=strict
- **Audit logging** — все действия панели логируются в `/var/log/vwn-panel-audit.log`
- **Безопасные обновления** — скачивание через tmpfile с проверкой shebang (без `curl | bash`)

## Веб-панель

### Установка

Панель устанавливается отдельно через пункт меню 32. При установке:
1. **Случайный URL путь** генерируется автоматически (напр. `/a1b2c3.../e5f6g7...`)
2. **Пароль** — выбор: ручной ввод или автогенерация (16 символов)
3. **URL** отображается после успешной установки

### Доступ

```
https://ваш-домен.com/<случайный_путь>/
```

Путь хранится в `/usr/local/etc/xray/panel.conf` и доступен через пункт меню 32 → 1.

### Возможности

- Dashboard со статусом сервисов, RAM/disk, uptime
- Управление пользователями (добавить/удалить/переименовать)
- Управление доменами (warp/relay/tor)
- Редактор конфигураций (xray, nginx, reality)
- Создание и скачивание бэкапов
- Просмотр логов
- Перезапуск сервисов
- Смена пароля
- Поддержка Basic Auth (.htpasswd)
...
└── panel.html    # Frontend веб-панели
...
├── panel.conf               # Настройки панели (secret, hash, path, port)
...
# Панель — посмотреть URL
vwn  # пункт 32 → пункт 1

# Панель — проверить логи
journalctl -u vwn-panel -n 50 --no-pager
```
