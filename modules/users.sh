...
buildUserSubFile() {
    ...
    if [ -f "$xhttpConfigPath" ]; then
        ...
        if [ -n "$x_domain" ] && [ -n "$x_uuid" ] && [ -n "$x_path" ]; then
            ...
            lines+="vless://${x_uuid}@${connect_host}:443?security=tls&type=xhttp&path=${x_enc_path}&mode=packet-up&alpn=h2&sni=${x_domain}&fp=chrome&allowInsecure=0#${x_encoded_name}"$'\n'
        fi
    fi
