...
# Единый список лог-файлов всех компонентов.
# Используется и в clearLogs() и в clear-logs.sh (cron).
# При добавлении нового компонента — добавить сюда И пересоздать cron (пункт меню).
_LOG_FILES=(
...
# Xray — все inbound'ы (WS, XHTTP, gRPC) пишут в один процесс
# USR1 заставляет xray переоткрыть лог-файлы после ротации
/var/log/xray/*.log {
    daily
    rotate 7
    missingok
...
/var/log/tor/*.log {
    ...
    sharedscripts
    postrotate
        systemctl kill -s HUP psiphon 2>/dev/null || true
    endscript
}
EOF
...
# Автоочистка логов
setupLogClearCron() {
    # Генерируем /usr/local/bin/clear-logs.sh динамически из _LOG_FILES
    # Так список файлов всегда синхронен — нет дублирования хардкода
    {
        echo '#!/bin/bash'
        echo '# Auto-generated by VWN logs.sh — do not edit manually'
        echo 'for f in \'
        local last_idx=$(( ${#_LOG_FILES[@]} - 1 ))
        local i
        for i in "${!_LOG_FILES[@]}"; do
            if [ "$i" -lt "$last_idx" ]; then
                echo "    ${_LOG_FILES[$i]} \\"
            else
                echo "    ${_LOG_FILES[$i]}; do"
            fi
        done
        echo '    [ -f "$f" ] && : > "$f"'
        echo 'done'
        echo 'journalctl --vacuum-size=50M &>/dev/null'
        echo 'journalctl --vacuum-time=7d &>/dev/null'
